...
export default function DirectionsMap({ isOpen, onClose, destination }: DirectionsMapProps) {
  ...
  // Polyline distance calculator
  const calculatePolylineDistance = (coords: [number, number][]) => {
  ...
  const fetchRoute = async (from: { lat: number; lng: number }, to: { lat: number; lng: number }) => {
    ...
    try {
      ...
      if (data.code === 'Ok' && data.routes?.length > 0) {
        ...
        // Compute accurate polyline distance
        const polylineKm = calculatePolylineDistance(coords);
        setDistance(polylineKm + ' km');
      ...
      >
        ...
        >
          ...
          <div className="bg-gradient-to-r from-blue-600 to-blue-700 p-4 flex items-center justify-between">
            ...
            <button className="text-black hover:bg-white/20 p-2 rounded-full" onClick={onClose}>
              <X className="w-6 h-6" />
            </button>
          </div>
          ...
          {currentLocation && distance && (
            <div className="bg-white border-b border-gray-200 p-4 flex items-center justify-between">
              ...
              <Button className="bg-blue-600 hover:bg-blue-700" size="sm" onClick={openInGoogleMaps}>
                Open in Google Maps
              </Button>
            </div>
          )}
          ...
          <div className="relative flex-1 bg-gray-100">
            ...
            {/* Legend */}
            <div className="absolute top-4 left-4 bg-white px-4 py-3 rounded-lg shadow-lg text-xs space-y-2">
              <div className="flex items-center space-x-2">
                <div className="w-4 h-4 rounded-full bg-blue-600 border-2 border-white"></div>
