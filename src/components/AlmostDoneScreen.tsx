...
//       authService.updateUserData({ username: username.trim() });
...
//                 className={`text-sm ${username.length >= 2 ? 'text-green-600' : 'text-gray-500'}`}
//               >
//                 {username.length >= 2 ? '✓ Looks good!' : 'Enter at least 2 characters'}
//               </motion.p>
//             )}
...
//               className="w-full bg-gradient-to-r from-pink-500 to-red-500 hover:from-pink-600 hover:to-red-600 text-white py-3 rounded-lg transition-all duration-200 flex items-center justify-center space-x-2"
//             >
...
//                     step <= 3 ? 'bg-pink-500' : 'bg-gray-300'
//                   }`}
//                 />
//               ))}
//             </div>
...
export default function AlmostDoneScreen({ onComplete }: AlmostDoneScreenProps) {
  ...
  const handleUsernameChange = (value: string) => {
    ...
    const formattedValue = cleanedValue.slice(0, 20);
    
    setUsername(formattedValue);
    
    // Validate and set error message
    if (formattedValue.trim().length < 3 && formattedValue.length > 0) {
      setError('Please enter at least 3 characters');
    } else if (formattedValue.trim().length === 0) {
      setError('');
    } else {
      setError('');
    }
            ...
            {username.length > 0 && (
              <div className="space-y-1">
                ...
                <motion.p
                  initial={{ opacity: 0, height: 0 }}
                  animate={{ opacity: 1, height: 'auto' }}
                  className={`text-sm ${isValidUsername ? 'text-green-600' : 'text-gray-500'}`}
                >
                  {isValidUsername ? '✓ Looks good!' : `${username.length}/3 characters`}
                </motion.p>
