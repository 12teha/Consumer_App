...
export default function PhoneEntryScreen({ onComplete, onBack }: PhoneEntryScreenProps) {
  const [phoneNumber, setPhoneNumber] = useState('');
  const [isLoading, setIsLoading] = useState(false);
  const [backgroundIndex, setBackgroundIndex] = useState(0);
  const [currentScreen, setCurrentScreen] = useState<'phone' | 'terms'>('phone');
  ...
  const handleBackClick = () => {
    if (onBack) {
      ...
      // Fallback: if no onBack prop provided, use browser back
      window.history.back();
    }
  };
    ...
    <div className="h-screen relative overflow-hidden">
      ...
      <div className="relative z-10 h-full flex flex-col justify-center p-8">
        <motion.div
          ...
          animate={{ opacity: 1, y: 0 }}
          transition={{ duration: 0.8, ease: "easeOut" }}
          className="bg-white/95 backdrop-blur-sm rounded-3xl p-8 shadow-2xl mx-auto w-full max-w-sm relative"
        >
          {/* Back Button */}
          {onBack && (
            <motion.button
              ...
              disabled={isLoading}
              className="absolute top-4 left-4 p-2 hover:bg-gray-100 rounded-full transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
            >
          ...
          >
            ...
            <Button
              onClick={handleSubmit}
              disabled={!isValidPhone || isLoading}
              className="w-full bg-gradient-to-r from-blue-500 to-purple-600 hover:from-blue-600 hover:to-purple-700 text-white py-3 rounded-lg transition-all duration-200 flex items-center justify-center space-x-2 disabled:opacity-50 disabled:cursor-not-allowed"
            >
              {isLoading ? (
                <motion.div
                  animate={{ rotate: 360 }}
