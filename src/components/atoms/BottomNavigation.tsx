...
const BottomNavigation = memo(function BottomNavigation({ activeScreen, onNavigate }: BottomNavigationProps) {
  ...
  const getIsActive = (itemId: string) => {
    return (
      activeScreen === itemId ||
      (itemId === 'home' &&
        ['home', 'offerDetails', 'allOffers', 'curatedOffers'].includes(activeScreen)) ||
      (itemId === 'profile' &&
        ['profile', 'profileDetails'].includes(activeScreen)) ||
      (itemId === 'storeListing' && activeScreen === 'storeListing')
    );
  };
